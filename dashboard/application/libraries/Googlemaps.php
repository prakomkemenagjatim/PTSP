...
class Googlemaps {
	...
	function create_map($addscript='')
	{
		...
		if ($this->maps_loaded == 0)
		{
			...
			else
			{
				...
				$apiLocation .= '.google.co.id/maps/api/js?';
			}
		...
		if ($this->bicyclingOverlay) {
  		...
  		$this->output_js_contents .=$this->scriptjs;
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		$data_location .= "maps.google.co.id/maps/api/geocode/json?address=".urlencode(utf8_encode($address))."&sensor=".$this->sensor;
