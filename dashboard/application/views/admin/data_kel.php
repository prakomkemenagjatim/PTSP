...
<div class="row">
  <div class="col-md-12 grid-margin stretch-card">
    <div class="card">
		<div class="card-body">			
			<div class="row">      
				<div class="col-md-3 col-xs-6 padding-lr-5 margin-t-10">                                       
				...
				<div data-options="name:'kelNama'">Kelurahan</div>
			...
			<div class="row">
		        <div class="col-md-12  padding-lr-5 margin-t-10 easyui-layout">
					<table id="dg" class="easyui-datagrid" style="height:400px"
							...
							<thead>
					            <tr>
					                <th data-options="field:'kecKode',width:120,align:'center'" sortable="true">KODE KEC</th>
					                <th data-options="field:'kelKode',width:100,align:'center'" sortable="true">KODE KEL</th>
...
<div id="modalkel" class="modal fade" role="dialog">
					...
					<div class="col-lg-12">
                    	...
                    	<div class="form-group">
	                        <label>Kecamatan</label>      
	                            <select class="form-control select" id="iKec" name="iKec" >
	                                <?php option_kec();?>
	                            </select>
                    	</div>
                        <div class="form-group">
                            <label>Kode Kelurahan (2 digit)</label>
                            <input id="iKelkode" name="iKelkode"  maxlength="2" type="text" class="form-control">
                        </div>
                        <div class="form-group">
                            <label>Nama Kelurahan</label>
                            <input id="iKelnama" name="iKelnama" type="text" class="form-control">
                        </div>
                     </div>			
					</div>                                 
                    </form>
...
<script type="text/javascript">
	...
	function loaddata(){
		$('#dg').datagrid({url:"<?php echo base_url('index.php/data/getkel')?>",queryParams:{
		...
		if (id == 2){
			...
			if (row){
				swal({
				  ...
				  var jpost = $.post("<?php echo base_url('index.php/data/delKel')?>",
									{
										idx:row.kelId
									},
									function(result){
			...
			if (id == 1){
			    ...
			    if (row){
					$('#idx').val(row.kelId);
					$('#iKelkode').val(row.kelKode);
					$('#iKelnama').val(row.kelNama);
					$('#iKec').val(row.kecKode);
					$('#modalkel').on('shown.bs.modal', function (e) {
					});
					
					$(".simpan").unbind('click');
					$(".simpan").click(function(){simpandata(id);})
					$('#modalkel').modal('toggle');
			    } else {
					swal({title: "Warning!!",text: 'Silakan Pilih Data Terlebih Dahulu.',type: "warning",});	
				}
		    } else {
	...
	function simpandata(id){
		if ($('#iKelkode').val().length != 2)
		{
			swal({title: "ERROR!!",text: 'Kode Kelurahan min/max 2 digit.',type: "error",});	
			return;
		}
		if (Number($('#iKelkode').val()) == 0)
		{
			swal({title: "ERROR!!",text: 'Kode Kelurahan tidak boleh 0.',type: "error",});	
			return;
		}
	  	url = "<?php echo base_url('index.php/data/postKel')?>";
		var jpost = $.post(url,
							{
								iKelkode:$('#iKelkode').val(),
								iKelnama:$('#iKelnama').val(),
								iKec:$('#iKec').val(),
								idx:$('#idx').val()
							},
							function(result){
