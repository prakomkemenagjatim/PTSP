                ...
                <div class="col-md-4 padding-r-5 margin-t-10">
					<div class="form-group">
						...
						<div id="mm" style="width:200px">
							...
							<div data-options="name:'ftanya'">Isi Konsultasi</div>
					...
					<table id="dg" class="easyui-datagrid" style="height:550px"
							...
							<thead>
								...
								<th  rowspan="2" data-options="field:'ck',checkbox:true"></th>
									...
									<th rowspan="2" data-options="field:'ftanya',width:500" sortable="true">ISI KONSULTASI</th>
									<th rowspan="2" data-options="field:'pemohonnama',width:300" sortable="true">NAMA LENGKAP</th>
									<th rowspan="2" data-options="field:'pemohonnowa',width:200" sortable="true">NO.HP/WA</th>
					                <th rowspan="2" data-options="field:'filedok',width:150,align:'center',formatter:formatlink" sortable="true">LINK FILE</th>
					            </tr>
								<tr>
									<th data-options="field:'nmstatus',width:150" sortable="true">STATUS</th>
									<th data-options="field:'ket',width:300" sortable="true">JAWABAN</th>
								</tr>
					        </thead>
					</table>
				</div>
										...
										<div class="row margin-b-15">
											...
											<div class="col-lg-12">
												<div class="form-group margin-b-15">
													...
													<div class="row">
														<div class="col-lg-12">
															<span id="tanya" name="tanya" class="form-control frmtxt"></span>
														</div>
													</div>
												</div>
											</div>
											...
											<style>
												...
												.lblket .rounded .text-warning{
													color: #212529 !important;

												}
												.lblket .rounded .text-danger, .lblket .rounded i,.lblket .rounded a{
													display: none !important;	
												}
											</style>
											<div class="col-lg-12 inpket">
												<div class="form-group margin-b-15">
													<label for="inputdefault" class="lblket">
														Link File
													</label>
													<div class="row">
														<div class="col-lg-12">
															<span id="fileproposal" name="fileproposal" class="form-control frmtxt"></span>
														</div>
													</div>
												</div>
											</div>
									...
									<form id="frmdok" action="" method="POST" enctype="multipart/form-data" >
										...
										<div class="col-lg-12 nodispo">
												<div class="form-group margin-b-15">
													<div class="row">
														<div class="col-lg-12">
															<textarea  rows="10" type="text" class="form-control form-text" id="ket" name="ket"></textarea>
														</div>
													</div>
												</div>
											</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){ 
	  ...
	  $('#dg').datagrid({url:"<?php echo base_url('index.php/data/getKonsul')?>",queryParams:{
	...
	function progressLay(dispo){
		...
		if (row){
			...
			$('#tanya').html(row.ftanya);
			$('#status').val(row.status);
			$('#ket').val(row.ket);
			$('#dok').html(row.hasildok);
			var urlfileproposal = '<?php echo viewdoc().str_replace('dashboard/','',base_url()) ; ?>'+row.filedok;
			if (row.filedok != '' && row.filedok != 'null'){
				$('#fileproposal').html('<a target="_blank" href="'+urlfileproposal+'" class="btn btn-info" style="border-radius: 50px;padding: 5px 20px;margin: 5px;" ><span class="fa fa-link fa-fw"></span>Link file</a>');
			} else {
				$('#fileproposal').html('-');
			}
	...
	function simpandata(){
		...
		$.ajax({
				url:  "<?php echo base_url('index.php/data/konsulPost')?>",
				type: 'POST',
				data: formData,
				async: false,
				cache: false,
	...
	function exportdata(){
		$.post('<?php echo base_url("index.php/export/excelKonsul"); ?>',{
			icari: scari.searchbox("getName"),
			ncari: scari.searchbox("getValue"),
			kd:kd,
			kd1:kd1,
