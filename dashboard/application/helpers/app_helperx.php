...
function alamatapp(){
	return 'Jl. Raden Panji Suroso Nomor 02 Malang 65126';
}

function footerapp(){
	return '©2021 Sistem Informasi Layanan Unggulan Masyarkat (SENYUM).';
}
function getCI(){
	$CI =& get_instance();
	return $CI; 
...
function optionDispojab(){
	$CI  = getCI();
	$CI->db->where("typeId between 2 and 11");
	$q = $CI->db->get("tbtypeuser")->result();
	$callback ='';
	if( count($q) > 0 ){
		foreach ($q as $key => $value)
...
function sendWA($phone_no,$message,$layanan=''){
	...
	$data = array("phone_no" => $phone_no, "key" => "7fd86bbc619352284b1eef3a83216ede35dc284d3d53290a", "message" => $message);
