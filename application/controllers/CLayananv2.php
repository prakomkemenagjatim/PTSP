...
class CLayananv2 extends Sitecontroller {
	...
	public function sekjen(){
		...
		if (count((array)$row) == 0){
			redirect('halaman/sekjen', 'refresh');
		}
		$jenis = $row->layananJenis;
		$judul = $row->menuJudul;
		$subjudul = $row->menuSubjudul;
		$template = $row->layananTemplate;
		cekviewsite($template,'halaman/sekjen');
		$data['content']='ld/forms/'.$template;
		$data['title'] = $jenis.' ('.$judul.') | KEMENTERIAN AGAMA KABUPATEN NGAWI';
		$data['head'] = $jenis;
	...
	public function kepegawaian(){
		...
		$row = $this->db->get("vwlayanan")->row();
		if (count((array)$row) == 0){
			redirect('halaman/kepegawaian', 'refresh');
		}
		$jenis = $row->layananJenis;
		$judul = $row->menuJudul;
		$subjudul = $row->menuSubjudul;
		$template = $row->layananTemplate;
		cekviewsite($template,'halaman/kepegawaian');
		$data['content']='ld/forms/'.$template;
		$data['title'] = $jenis.' ('.$judul.') | KEMENTERIAN AGAMA KABUPATEN NGAWI';
		$data['head'] = $jenis;
		$data['img'] = $row->layananimg;
	...
	public function kua_keren(){
		...
		$row = $this->db->get("vwlayanan")->row();
		if (count((array)$row) == 0){
			redirect('page/kua-keren', 'refresh');
		}
		$jenis = $row->layananJenis;
		$judul = $row->menuJudul;
		$subjudul = $row->menuSubjudul;
		...
		// Fetch all rows from vwsatkeraktif where SATKER_2 contains 'KUA'
		$this->db->like('SATKER_2', 'KUA');
		$kua_data = $this->db->get('vwsatkeraktif')->result_array();
		$data['kua_data'] = $kua_data;
