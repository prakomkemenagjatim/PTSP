...
Simpan Nomor ini jika link tidak aktif";
			...
			if ( $upload1['error'] != '' ){
				...
				if ( $upload2['error'] != '' ){
					...
					if ($result){
						$row = $result->row();
						$ahref= '<a href="'.base_url('index.php/page/lacak-layanan').'" target="_blank"> <i>disini</i> </a>';
						$ndata['success'] = 'Nomor Layanan Anda: <p><strong><span class="h2 mt-2 red-text">'.$row->noreg.'</span></strong></p>Nomor WA/HP: <p><strong><span class="h5 mt-2 red-text">'.$nowa.'</span></strong></p><p><strong><sup>Silakan catat nomor layanan untuk lacak layanan '.$ahref.'.</sup></strong>';
						$ndata['error'] = '';
						$this->sendfire($row->noreg);
						$this->sendwa($nowa,$row->noreg,$perihalsurat);
					} else {
			...
			if (count($row) > 0){
				if (trim($row->pemohonnowa) == trim($nowa)){
					if ($row->filedok == '' || $row->filedok == null ){
						...
						$filedok = 'Silakan download dokumen dengan klik tombol download dibawah ini.<a target="_blank" href="'.$href.'" class="btn btn-info" style="border-radius: 50px;padding: 5px 20px;margin: 5px;" ><span class="fa fa-link fa-fw"></span>Download</a>';
					}
