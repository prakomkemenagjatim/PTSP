...
class CData extends Sitecontroller {
	...
	function simpanbukutamu(){
		...
		$json=array();
		if ($bt_nama == '' || $bt_alamat == '' || $bt_hp == '' || $bt_layanan == '' || $bt_layanan == '0' ){
			$json['status']=2;
		} else {
...
// 			$fd = './dataupload/';
// 			$berkasfd = './dataupload/dokfile/';
// 			$berkasfd1 = '/dataupload/dokfile/';
			...
			if (count($listfile) == count($listberkasnm)){
				...
				if ($result){
					$row = $result->row();
					$queri= "INSERT INTO tbdisposisi (disposisi_kode,disposisi_noreg,disposisi_dari,disposisi_ke) VALUES(CONCAT($noreg,'_2'),$noreg,1,4)";
					$noreg = $row->noreg;
...
// 			$berkasfd1 = '/dataupload/dokfile/';
			...
			if (count($listfile) == count($listberkasnm)){
				...
				if ($result){
					...
					$this->sendwakua($nowa,$row->noreg,$perihalsurat);
					// Fetch userWA from tbl_user
					$userQuery = $this->db->query("SELECT userWA FROM tbuser WHERE KODE_SATKER_2 = '$tujuansurat'");
					if ($userQuery->num_rows() > 0) {
						$userRow = $userQuery->row();
						$nowakua = $userRow->userWA;
						$this->sendwakua($nowakua, $row->noreg, $perihalsurat);
					}
				} else {
					$ndata['error'] = 'Gagal mengirim data.';
					$ndata['success'] = '';
...
function pengajuanPost(){
    ...
    private function sendwapengajuan($nowa,$nolayanan,$perihalsurat){
		...
		$msg2 = "✅ *Permohonan Masuk Layanan SANTUN KEMENAG Kabupaten Ngawi*
...
Kementerian Agama Kabupaten Ngawi, Hebat Bermartabat";
	...
	private function sendwa($nowa,$nolayanan,$perihalsurat){
		if ($perihalsurat != ''){
			$halsurat = '✉️ Perihal Surat: *'.$perihalsurat.'*';
			$halsurat2 = '✉️ Perihal Surat: '.$perihalsurat;
		} else {
			$halsurat='';
		}
		
		$msg2 = "Layanan Masuk Santun Kemenag Ngawi
✉️ No.Layanan: $nolayanan
✉️ No.WA/HP: $nowa
$halsurat2
";
...
Kementerian Agama Kabupaten Ngawi, Hebat Bermartabat";
	...
	private function sendwakua($nowa,$nolayanan,$perihalsurat){
		if ($perihalsurat != ''){
			$halsurat = '✉️ Perihal Surat: *'.$perihalsurat.'*';
			$halsurat2 = '✉️ Perihal Surat: '.$perihalsurat;
		} else {
		...
		$msg = "📢 * Layanan KUA KEREN KEMENAG Kabupaten Ngawi 
(Kredibel, Moderat, Responsif dan Kompeten)	
...
Kementerian Agama Kabupaten Ngawi, Hebat Bermartabat";
	...
	function getjadwalsholat(){
		...
		curl_setopt( $ch,CURLOPT_POSTFIELDS, "param_prov=c74d97b01eae257e44aa9d5bade97baf&param_kabko=03c6b06952c750899bb03d998e631860&param_thn=".$thn."&param_bln=".$bln."&param_token=af7c667b9819378c0bddb3baede9525b" );
		$result = curl_exec($ch );
		echo $result;
		curl_close( $ch );
	}
	...
	function getjadwalimsak(){
		...
		curl_setopt( $ch,CURLOPT_RETURNTRANSFER, true );
		curl_setopt( $ch,CURLOPT_SSL_VERIFYPEER, false );
		curl_setopt( $ch,CURLOPT_POSTFIELDS, "param_prov=c74d97b01eae257e44aa9d5bade97baf&param_kabko=03c6b06952c750899bb03d998e631860S&param_thn=".$thn."&param_token=af7c667b9819378c0bddb3baede9525b" );
		$result = curl_exec($ch );
		echo $result;
		curl_close( $ch );
	}
	...
	function getGuruagamakristen(){
		...
		$sort = isset($_POST['sort']) ? $_POST['sort'] : 'guru_nip';
		$order = isset($_POST['order']) ? strval($_POST['order']) : 'desc';
		$icari = isset($_POST['icari']) ? $_POST['icari'] : 'guru_nama';
		$ncari = isset($_POST['ncari']) ? addslashes($_POST['ncari']) : '';
		$kec = isset($_POST['skec']) ? addslashes($_POST['skec']) : '';
		$kel = isset($_POST['kel']) ? addslashes($_POST['kel']) : '';
		$status = isset($_POST['sstatus']) ? addslashes($_POST['sstatus']) : '';
		$jenjang = isset($_POST['sjenjang']) ? addslashes($_POST['sjenjang']) : '';
		$sertifikasi = isset($_POST['ssertifikasi']) ? addslashes($_POST['ssertifikasi']) : '';
		$offset = ($page-1)*$rows;
		$where = '';
		$wherewil = '';
		if ($kec != ''){
		...
		$wherejenjang = '';
		if ($jenjang != ''){
			$wherejenjang = ' and sekolah_jenjang = "'.$jenjang.'"';
		}

		$whereser = '';
		if ($sertifikasi != ''){
			$whereser = ' and guru_sertifikasi = "'.$sertifikasi.'"';
		}
		...
		$where = " where guru_tipe = '1' and $icari like '%$ncari%' ".$wherewil.$wherestatus.$wherejenjang.$whereser;		
		$result = array();
		$tabel = "SELECT * from vwguruagama_public ";
	...
	function grafikGuruagamakristen(){
		$icari = isset($_POST['icari']) ? $_POST['icari'] : 'nama';
		$ncari = isset($_POST['ncari']) ? addslashes($_POST['ncari']) : '';
		$kec = isset($_POST['skec']) ? addslashes($_POST['skec']) : '';
		$kel = isset($_POST['kel']) ? addslashes($_POST['kel']) : '';
		$status = isset($_POST['sstatus']) ? addslashes($_POST['sstatus']) : '';
		$jenjang = isset($_POST['sjenjang']) ? addslashes($_POST['sjenjang']) : '';
		$dasar = isset($_POST['sdasar']) ? addslashes($_POST['sdasar']) : 'status';
		$sertifikasi = isset($_POST['ssertifikasi']) ? addslashes($_POST['ssertifikasi']) : '';
		...
		$field = $dasar;
		if ($dasar == 'jenjang'){
			$field = 'sekolah_jenjang';
			$group = $field;
			$judul = 'Jenjang Sekolah';
		} else
		if ($dasar == 'sertifikasi'){
			$field = 'guru_sertifikasi';
			$group = $field;
			$judul = 'Sertifikasi';
		} else
		if ($dasar == 'status'){
			$field = 'guru_status';
			$group = $field;
			$judul = 'Status Guru Agama Kristen';
		} else
		if ($dasar == 'kec'){
			$field = 'kecNama';
			$group = $field;
			$judul = 'Wilayah Kecamatan Guru Agama Kristen';
		}  
		$where = " where guru_tipe = '1' and $icari like '%$ncari%' ".$wherewil.$wherejenjang.$wherestatus.$whereser;
		$tabel = "SELECT $field as nama, count(*) as jml from vwguruagama_public ";
		/*$query = $this->db->query($tabel.$where);
		$result["total"] = $query->num_rows();*/
		$rs = $tabel.$where." group by $group ";
		$rs = $this->db->query($rs);
		$items = $rs->result_array();
		//echo $this->db->last_query();
		$data = array();
			foreach ($items as $row)    {
				array_push($data,array('y'=>(int)$row['jml'],'name'=>$row['nama']));
			}
		$datax['data'] = $data;
		$datax['judul'] = 'Berdasarkan '.$judul;	
		$json = json_encode($datax);	
		die($json);
	}
	...
	function getGuruagamakatolik(){
		...
		$where = " where guru_tipe = '2' and $icari like '%$ncari%' ".$wherewil.$wherestatus.$wherejenjang.$whereser;		
		$result = array();
		$tabel = "SELECT * from vwguruagama_public ";
		$query = $this->db->query($tabel.$where);
		$result["total"] = $query->num_rows();
	...
	function grafikGuruagamakatolik(){
		$icari = isset($_POST['icari']) ? $_POST['icari'] : 'nama';
		$ncari = isset($_POST['ncari']) ? addslashes($_POST['ncari']) : '';
		$kec = isset($_POST['skec']) ? addslashes($_POST['skec']) : '';
		$kel = isset($_POST['kel']) ? addslashes($_POST['kel']) : '';
		...
		if ($dasar == 'status'){
			$field = 'guru_status';
			$group = $field;
			$judul = 'Status Guru Agama Katolik';
		} else
		if ($dasar == 'kec'){
			$field = 'kecNama';
			$group = $field;
			$judul = 'Wilayah Kecamatan Guru Agama Katolik';
		}  
		$where = " where guru_tipe = '2' and $icari like '%$ncari%' ".$wherewil.$wherejenjang.$wherestatus.$whereser;
		$tabel = "SELECT $field as nama, count(*) as jml from vwguruagama_public ";
		/*$query = $this->db->query($tabel.$where);
		$result["total"] = $query->num_rows();*/
		$rs = $tabel.$where." group by $group ";
